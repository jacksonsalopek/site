...
export const animationState = {
  ...
  effect: undefined,
  lights: [new PointLight(16777215, 1), new PointLight(16777215, 0.5)],
...
export function createEffect() {
  ...
  const effect = new AsciiEffect(animationState.renderer, animationState.config.charSet, {
    invert: true,
    resolution: animationState.config.resolution
  });
  const width = window.innerWidth - window.innerWidth * 80 / window.innerHeight;
  const height = window.innerHeight - 80;
  effect.setSize(width * 0.5, height * 0.5);
  effect.domElement.style.color = animationState.config.color;
  effect.domElement.style.backgroundColor = animationState.config.backgroundColor;
  return effect;
}
export function render(effect) {
...
export function tick() {
  if (animationState.effect) {
    if (animationState.rotate) {
      animationState.mesh.rotation.z = animationState.clock.getElapsedTime() / 3;
      if (animationState.config.oscillate)
        animationState.mesh.rotation.y = animationState.config.amplitude * Math.sin(animationState.clock.getElapsedTime() / 3);
...
export function onLoad(geometry) {
  ...
  if (boundingBox) {
    animationState.mesh.position.y = (boundingBox.max.z - boundingBox.min.z) / 5;
...
export function bootstrap() {
  ...
  animationState.scene.background = new Color(animationState.config.backgroundColor);
